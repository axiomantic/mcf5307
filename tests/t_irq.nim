...
##
##   1. THE MASK BOUNDARY, IN BOTH DIRECTIONS. Interrupt requests are
##      inhibited for all priority levels less than or equal to the current
##      priority. Blocks 1 and 2 present the SAME level 3
...
##
##   2. THAT NOTHING LATCHES AT LEVELS 1 TO 6. Interrupt levels 1 through 6 are
##      level-sensitive only. Block 3 runs
...
##      only the flag. 66 is in the user-defined range 64-255 and its slot holds
##      a DIFFERENT
...
##
##   4. THE LEVEL-7 CASES. A level 7 interrupt is nonmaskable, so a 7 in the
##      interrupt mask does not disable it, and it is edge triggered by a
##      transition from a lower priority request to the level 7 request, so if
##      IRQ7 remains asserted only one level 7 interrupt is recognized. Blocks
##      6 to 9 carry the cases the edge rule names.
##
...
##      MOMENT OF THE CALL, so that the fixed position - after the frame, after
##      the mask write, before the first handler
##      instruction - is a value in an assertion and not a sentence in a
##      comment.
##
##   7. THE DIVERGENCE THIS PROJECT CHOSE. Block 12 runs the manual's SECOND
...
##      HANDLERS. Sampling is inhibited during the first instruction of ALL
##      exception handlers, and block 15
...
##      one no instruction enters: RESET is itself an exception, so the
##      instruction at the reset program
...
##      PROCESS. Nothing aborts the process, because an abort inside a plugin
##      destroys the host's session. Block 21 is two
...
##     the stacking faults, which is to stop. The exception the MCF5307
##     actually takes there is not modelled, and this file will need a case for
##     it.
...
##     instruction of this tree can take, and moving the write out of
##     `takeException` and into `execTrap`, which turns the rule into a rule
##     about one instruction, leaves this file green. The funnel is a property
##     of WHERE the line sits and not a measurement. The case that would
##     separate the two arrives with the bus-fault exception, which is the
##     second path into `takeException` from inside `step`. BLOCK 22 IS NOT
##     THAT CASE and does not weaken this entry:
...
## about Motorola silicon, taken from the MCF5307 User's Manual.
...
const
  ...
  opTrapZeroWord = 0x4E40'u16
    ...
    ## inside `step`. The bus-fault exception is the second, and it
    ## does not exist yet.
...
type
  ...
  # the first handler instruction is fetched, and the module header of
  # `src/mcf5307/irq.nim` justifies that position against the hardware's own
  # acknowledge cycle, which comes SECOND instead. A tuple of level and vector
  # alone RECORDS NONE OF THAT: moving the acknowledge to before
  # `takeException`, and moving it to before the mask write, are each invisible
  # to one. The four fields below
...
#      and take nothing.
#
# A level source drops when the device model clears its own condition, and
# nothing in the emulator drops it on the firmware's behalf.
...
# differ in the NUMBER OF CALLS and in
...
#
# A transition to level 7 arms one non-maskable interrupt, and the level
# dropping back does not disarm it.
#
...
# EDGE HALF AND NOT THE LEVEL HALF, because the project's rule is edge-only and
# the G2 programs no level-7 source. THE DECLARATION HAD NO TEST: deleting
# `and level <= 6` from `pendingInterrupt` once reddened nothing, and that
# deletion is
...
# test: deleting the `if ctx.halted: return true` after `takeException`,
# deleting `cpu.nim`'s halted break, and moving the level-7 latch clear to
# after the halted check were each invisible, because every asserted `halted`
# was `false` and nothing reached the path at all.
#
...
# and `takeException` returns early with the context halted. What a DOUBLE
# FAULT should then DO is undecided; what this block pins is only what today's
# core does, which is: stop.
#
...
# stacking - a double fault - does not leave an interrupt armed that the
...
# PROCESS.
#
# Nothing aborts the process, because an abort inside a plugin destroys the
# host's session. `include/mcf5307.h` forbids no
