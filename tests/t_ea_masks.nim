...
proc checkImpl(cond: bool; label: string) =
  if cond:
    echo "PASSED  ", label
    inc passCount
  else:
...
template check(cond: bool; label: string) =
  checkImpl(cond, label)

template checkOffGreenPath(cond: bool; label: string) =
  ## The one call in this suite a green run does not reach. Reaching it IS
  ## the failure: it reports a table entry that should exist and does not,
  ## so the suite is already red by the time this site runs.
  checkImpl(cond, label)
proc checkDetailImpl(cond: bool; label: string; got: string) =
  if cond:
    echo "PASSED  ", label
    inc passCount
  else:
...
template checkDetail(cond: bool; label: string; got: string) =
  checkDetailImpl(cond, label, got)
# ---------------------------------------------------------------------------
