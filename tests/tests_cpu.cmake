...
#
# Cases 1 and 2 build a static library and link no executable, so they catch a
# header that does not compile and catch NO rename at all. Cases 3 and 4 link,
# which is what turns a renamed declaration into an error rather than into
# nothing, and the stub is what makes them linkable without the real
# implementation.
#
# THE FOUR CASES ARE TARGETS OF A NESTED CMAKE PROJECT, AND THE TEST BUILDS
# THEM. This file used to spell the four compiler commands itself, in GNU
# flags. MSVC accepts `-` in front of an option, so those did not fail cleanly:
# `-Wextra` arrived as `/Wextra` and `cl` answered `command line error D8021:
# invalid numeric argument 'extra'`. Translating every spelling was not enough
# either -- `cl.exe` started from `execute_process` inherits no `%INCLUDE%` and
# no `%LIB%`, because `vcvarsall.bat` is what sets those and nothing here runs
# it, so the next failure was `fatal error C1034: stddef.h: no include path
# set` on the contract header's own first include. Both measured on
# windows-2022. `cmake/Nim.cmake` step 4a met the same wall and took the same
# way out: CMake knows the toolchain, so CMake is given the job, and the
# warning set, the language standard and the link line are each spelled by the
# compiler that reads them.
#
# THE COMPILE AND THE LINK STILL HAPPEN INSIDE THE TEST. That is the property
# the hand-written commands existed for. A `ctest` run over a tree whose build
# had failed must not run a STALE executable from the previous build and report
# a pass, so each case passes `--clean-first` and the compile it asserts on
# really runs. `--clean-first` cleans the whole nested project and not one
# target, which costs nothing here because every case rebuilds what it reads.

set(abi_contract_src "${CMAKE_CURRENT_BINARY_DIR}/abi_contract")
set(abi_contract_bin "${CMAKE_CURRENT_BINARY_DIR}/abi_contract-build")
set(abi_contract_out "${CMAKE_CURRENT_BINARY_DIR}/abi_contract-out")
file(MAKE_DIRECTORY "${abi_contract_src}")
file(MAKE_DIRECTORY "${abi_contract_out}")

# The two units of cases 1 and 2. Each includes the contract header and nothing
# else, and defines one ordinary function: an object with no symbol in it makes
# the librarian warn and says nothing about the header.
file(WRITE "${abi_contract_src}/header_only.c"
"/* GENERATED BY tests/tests_cpu.cmake. Do not edit this copy in the build\n"
" * tree. It is case 1 of t0_abi_header: the contract header, alone, as C11. */\n"
"#include \"mcf5407.h\"\n"
"int mcf5407_header_only_c(void) { return 0; }\n")
file(WRITE "${abi_contract_src}/header_only.cpp"
"/* GENERATED BY tests/tests_cpu.cmake. Do not edit this copy in the build\n"
" * tree. It is case 2 of t0_abi_header: the same header, as C++17. */\n"
"#include \"mcf5407.h\"\n"
"int mcf5407_header_only_cxx(void) { return 0; }\n")

set(abi_contract_template [==[
# GENERATED BY tests/tests_cpu.cmake. Do not edit this copy in the build tree.
# It holds the four cases of the registered test `t0_abi_header`, one target
# each, so that CMake and not that file spells the compiler flags.

cmake_minimum_required(VERSION 3.26)
project(mcf5407_abi_contract C CXX)

# Every configuration writes to ONE directory, so the driver can name the two
# executables without knowing which configuration the generator chose.
set(CMAKE_RUNTIME_OUTPUT_DIRECTORY "@abi_contract_out@")
set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY "@abi_contract_out@")
set(CMAKE_RUNTIME_OUTPUT_DIRECTORY_DEBUG "@abi_contract_out@")
set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY_DEBUG "@abi_contract_out@")
set(CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELEASE "@abi_contract_out@")
set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY_RELEASE "@abi_contract_out@")
set(CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELWITHDEBINFO "@abi_contract_out@")
set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY_RELWITHDEBINFO "@abi_contract_out@")
set(CMAKE_RUNTIME_OUTPUT_DIRECTORY_MINSIZEREL "@abi_contract_out@")
set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY_MINSIZEREL "@abi_contract_out@")

# `/W4` and not `/Wall`. MSVC's `/Wall` turns on every warning it has,
# including the ones its own system headers raise, and `/WX` would then fail
# every target here over `<stdio.h>`. `/W4` is the level that corresponds to
# what `-Wall -Wextra` selects, and `/permissive-` is the standard conformance
# `-pedantic` asks for.
if(MSVC)
    set(abi_contract_flags /W4 /permissive- /WX)
else()
    set(abi_contract_flags -Wall -Wextra -pedantic -Werror)
endif()

add_library(abi_header_c STATIC "@abi_contract_src@/header_only.c")
add_library(abi_header_cxx STATIC "@abi_contract_src@/header_only.cpp")

# `abi_stub.c` is compiled AS C in both executables, because that is what it
# is. Case 4 is the one that depends on it: the header's `extern "C"` block is
# what makes the C++ side link against a C definition, and a header that lost
# it fails there on a mangled name.
add_executable(abi_surface_c
    "@ABI_TEST_SRC_DIR@/t0_abi_header.c"
    "@ABI_TEST_SRC_DIR@/abi_stub.c")
add_executable(abi_surface_cxx
    "@ABI_TEST_SRC_DIR@/t0_abi_header.cpp"
    "@ABI_TEST_SRC_DIR@/abi_stub.c")

foreach(abi_contract_target
        abi_header_c abi_header_cxx abi_surface_c abi_surface_cxx)
    target_include_directories(${abi_contract_target} PRIVATE
        "@ABI_TEST_INCLUDE_DIR@")
    target_compile_options(${abi_contract_target} PRIVATE
        ${abi_contract_flags})
    # `*_EXTENSIONS OFF` is what asks for `c11` rather than `gnu11`, which is
    # the standard the contract header is required to compile under.
    set_target_properties(${abi_contract_target} PROPERTIES
        C_STANDARD 11
...
set(ABI_TEST_SRC_DIR "${CMAKE_CURRENT_LIST_DIR}")
set(ABI_TEST_INCLUDE_DIR "${PROJECT_SOURCE_DIR}/include")
string(CONFIGURE "${abi_contract_template}" abi_contract_text @ONLY)
file(WRITE "${abi_contract_src}/CMakeLists.txt" "${abi_contract_text}")

# The nested project is configured here and built by the test. The generator is
# forwarded because a different one would measure a different toolchain, and a
# multi-configuration generator selects its compiler from the platform and the
# toolset rather than from a compiler path -- so those are what it is given,
# and every other generator is given the compilers directly.
set(abi_contract_configure_args -G "${CMAKE_GENERATOR}")
if(CMAKE_GENERATOR MATCHES "^(Visual Studio|Xcode)")
    if(CMAKE_GENERATOR_PLATFORM)
        list(APPEND abi_contract_configure_args
...
execute_process(
    COMMAND "${CMAKE_COMMAND}" -S "${abi_contract_src}"
            -B "${abi_contract_bin}" ${abi_contract_configure_args}
    OUTPUT_VARIABLE abi_contract_configure_out
    ERROR_VARIABLE abi_contract_configure_err
    RESULT_VARIABLE abi_contract_configure_rc)
if(NOT abi_contract_configure_rc EQUAL 0)
    message(FATAL_ERROR
        "tests: t0_abi_header cannot be registered: the nested contract "
        "project did not configure.\n"
        "  project   : ${abi_contract_src}/CMakeLists.txt\n"
        "  generator : ${CMAKE_GENERATOR}\n"
        "  exit      : ${abi_contract_configure_rc}\n"
        "  stdout    : ${abi_contract_configure_out}\n"
        "  stderr    : ${abi_contract_configure_err}\n"
        "That project holds the four cases of the contract check. Registering "
        "the test without it would register a test that cannot build what it "
        "asserts on, and a test that cannot run is not a check.")
endif()

set(abi_contract_exe_c
    "${abi_contract_out}/abi_surface_c${CMAKE_EXECUTABLE_SUFFIX}")
set(abi_contract_exe_cxx
    "${abi_contract_out}/abi_surface_cxx${CMAKE_EXECUTABLE_SUFFIX}")

# The four cases run from a driver script that this file writes into the build
# tree. It runs all four, reports every one by name, and fails if any failed.
...
# A multi-configuration generator needs the configuration named; a
# single-configuration one rejects an empty `--config`.
set(abi_config_args "")
if(NOT ABI_CONFIG STREQUAL "")
    set(abi_config_args --config "${ABI_CONFIG}")
endif()

# A case is one label and one or more STEPS. The first step that fails ends
# the case: the later steps are skipped, because running an executable that did
# not build reports a second failure that says nothing the first did not
# already say. Skipping the run after a failed build is also what stops a STALE
# executable from an earlier run being executed and reported as a pass.
...
# `--clean-first` is the whole reason this builds rather than runs. Without it
# an up-to-date target reports success having compiled nothing, and the compile
# is the assertion.
macro(abi_build abi_target)
    if(abi_rc STREQUAL "0")
        execute_process(
            COMMAND "${ABI_CMAKE}" --build "${ABI_BUILD_DIR}"
                    ${abi_config_args} --target "${abi_target}" --clean-first
...
macro(abi_run abi_executable)
    if(abi_rc STREQUAL "0")
        if(NOT EXISTS "${abi_executable}")
            set(abi_rc "no such executable: ${abi_executable}")
        else()
            execute_process(
                COMMAND "${abi_executable}"
                RESULT_VARIABLE abi_rc
                OUTPUT_VARIABLE abi_out
                ERROR_VARIABLE abi_err)
        endif()
...
# `$<CONFIG>` is what carries the configuration a multi-configuration generator
# chose at build time. It cannot be read at configure time, and `add_test`
# expands it.
add_test(NAME t0_abi_header
    COMMAND "${CMAKE_COMMAND}"
        "-DABI_CMAKE=${CMAKE_COMMAND}"
        "-DABI_BUILD_DIR=${abi_contract_bin}"
        "-DABI_CONFIG=$<CONFIG>"
        "-DABI_EXE_C=${abi_contract_exe_c}"
        "-DABI_EXE_CXX=${abi_contract_exe_cxx}"
