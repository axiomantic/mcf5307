...
# THE DRIVER'S COMPILER SPELLINGS. They are decided here, where the compiler
# is known, and written into the driver rather than passed to it: the driver is
# a `cmake -P` script whose arguments already carry six paths, and a spelling
# that must survive a `-D` on a command line cannot be a list.
#
# MSVC accepts `-` and `/` interchangeably in front of an option, so a GNU
# spelling does not simply fail -- `-Wextra` arrives as `/Wextra`, which `cl`
# reads as `/W` with a numeric argument of `extra`. Measured on windows-2022:
# `cl : Command line error D8021 : invalid numeric argument '/Wextra'`, four
# cases out of four, before any of them compiled a line.
#
# `/W4` and not `/Wall`. MSVC's `/Wall` turns on every warning the compiler
# has, including the ones its own system headers raise, and `/WX` then fails the
# case over `<stdio.h>`. `/W4` is the level that corresponds to what
# `-Wall -Wextra` selects.
if(MSVC)
    set(abi_spelling_flags "/W4 /permissive- /WX")
    set(abi_spelling_std_c "/std:c11")
    set(abi_spelling_std_cxx "/std:c++17")
    set(abi_spelling_syntax_only "/Zs")
    set(abi_spelling_as_c "/TC")
    set(abi_spelling_as_cxx "/TP")
    set(abi_spelling_compile_only "/c")
    set(abi_spelling_include "/I")
    set(abi_spelling_obj_out "/Fo")
    set(abi_spelling_exe_out "/Fe")
    set(abi_spelling_obj_suffix ".obj")
    set(abi_spelling_exe_suffix ".exe")
    # `cl` joins an output path to its flag; a GNU driver takes it as the next
    # argument. One boolean is what keeps the four call sites free of the split.
    set(abi_spelling_output_joined TRUE)
else()
    set(abi_spelling_flags "-Wall -Wextra -pedantic -Werror")
    set(abi_spelling_std_c "-std=c11")
    set(abi_spelling_std_cxx "-std=c++17")
    set(abi_spelling_syntax_only "-fsyntax-only")
    set(abi_spelling_as_c "-x c")
    set(abi_spelling_as_cxx "-x c++")
    set(abi_spelling_compile_only "-c")
    set(abi_spelling_include "-I")
    set(abi_spelling_obj_out "-o")
    set(abi_spelling_exe_out "-o")
    set(abi_spelling_obj_suffix ".o")
    set(abi_spelling_exe_suffix "")
    set(abi_spelling_output_joined FALSE)
endif()

# `@ONLY` is what lets the template below keep its own `${...}` references. The
# driver runs in a separate `cmake -P` process and evaluates them there.
set(abi_driver_template [==[
...
# Written by tests/tests_cpu.cmake from the compiler CMake selected. The list
# forms are split from single strings here so that a spelling made of two words
# -- `-x c` -- reaches the compiler as two arguments.
separate_arguments(abi_flags NATIVE_COMMAND "@abi_spelling_flags@")
separate_arguments(abi_as_c NATIVE_COMMAND "@abi_spelling_as_c@")
separate_arguments(abi_as_cxx NATIVE_COMMAND "@abi_spelling_as_cxx@")
set(abi_std_c "@abi_spelling_std_c@")
set(abi_std_cxx "@abi_spelling_std_cxx@")
set(abi_syntax_only "@abi_spelling_syntax_only@")
set(abi_compile_only "@abi_spelling_compile_only@")
set(abi_include_flag "@abi_spelling_include@")
set(abi_obj_out_flag "@abi_spelling_obj_out@")
set(abi_exe_out_flag "@abi_spelling_exe_out@")
set(abi_obj_suffix "@abi_spelling_obj_suffix@")
set(abi_exe_suffix "@abi_spelling_exe_suffix@")
set(abi_output_joined @abi_spelling_output_joined@)

# An output argument is one word under `cl` and two under a GNU driver.
macro(abi_output abi_output_var abi_output_flag abi_output_path)
    if(abi_output_joined)
        set(${abi_output_var} "${abi_output_flag}${abi_output_path}")
    else()
        set(${abi_output_var} "${abi_output_flag}" "${abi_output_path}")
    endif()
endmacro()
...
macro(abi_step)
    if(abi_rc STREQUAL "0")
        # The working directory is named because `cl` drops an intermediate
        # object beside the CURRENT directory, which under `ctest` is the build
        # tree root. Every path a step is given is absolute, so naming it
        # changes nothing else.
        execute_process(COMMAND ${ARGN}
                        WORKING_DIRECTORY "${ABI_WORK_DIR}"
...
abi_step("${ABI_C_COMPILER}" "${abi_std_c}" "${abi_syntax_only}" ${abi_flags}
         ${abi_as_c} "${ABI_HEADER}")
abi_end_case()

abi_begin_case("case 2: include/mcf5407.h compiles as C++17")
abi_step("${ABI_CXX_COMPILER}" "${abi_std_cxx}" "${abi_syntax_only}"
         ${abi_flags} ${abi_as_cxx} "${ABI_HEADER}")
abi_end_case()
...
set(abi_stub_c "${ABI_WORK_DIR}/abi_stub_for_c${abi_obj_suffix}")
set(abi_exe_c "${ABI_WORK_DIR}/t0_abi_header_c${abi_exe_suffix}")
abi_output(abi_stub_c_out "${abi_obj_out_flag}" "${abi_stub_c}")
abi_output(abi_exe_c_out "${abi_exe_out_flag}" "${abi_exe_c}")
abi_step("${ABI_C_COMPILER}" "${abi_std_c}" ${abi_flags}
         "${abi_include_flag}${ABI_INCLUDE_DIR}"
         "${abi_compile_only}" ${abi_stub_c_out} "${ABI_SRC_DIR}/abi_stub.c")
abi_step("${ABI_C_COMPILER}" "${abi_std_c}" ${abi_flags}
         "${abi_include_flag}${ABI_INCLUDE_DIR}"
         ${abi_exe_c_out}
         "${ABI_SRC_DIR}/t0_abi_header.c" "${abi_stub_c}")
abi_step("${abi_exe_c}")
abi_end_case()
...
set(abi_stub_cpp "${ABI_WORK_DIR}/abi_stub_for_cpp${abi_obj_suffix}")
set(abi_exe_cpp "${ABI_WORK_DIR}/t0_abi_header_cpp${abi_exe_suffix}")
abi_output(abi_stub_cpp_out "${abi_obj_out_flag}" "${abi_stub_cpp}")
abi_output(abi_exe_cpp_out "${abi_exe_out_flag}" "${abi_exe_cpp}")
abi_step("${ABI_C_COMPILER}" "${abi_std_c}" ${abi_flags}
         "${abi_include_flag}${ABI_INCLUDE_DIR}"
         "${abi_compile_only}" ${abi_stub_cpp_out}
         "${ABI_SRC_DIR}/abi_stub.c")
abi_step("${ABI_CXX_COMPILER}" "${abi_std_cxx}" ${abi_flags}
         "${abi_include_flag}${ABI_INCLUDE_DIR}"
         ${abi_exe_cpp_out}
         "${ABI_SRC_DIR}/t0_abi_header.cpp"
         "${abi_stub_cpp}")
abi_step("${abi_exe_cpp}")
abi_end_case()
...
string(CONFIGURE "${abi_driver_template}" abi_driver_text @ONLY)
