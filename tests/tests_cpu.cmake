...
# the address of every function `include/mcf5307.h` declares AND THE LIBRARY
# DEFINES, which is the assertion that a renamed or dropped definition is a
# link error. It calls `mcf5307_runtime_init()` twice and asserts both calls
# return. C++ never names `NimMain`; it calls `mcf5307_runtime_init()`, which
# is idempotent.
#
...
# which link it against `tests/abi_stub.c`. A name nothing
# defines cannot be renamed and cannot be dropped, so asserting it here buys
# no coverage that the stub cases do not already give.
#
...
# variable is what makes this test GROW ON ITS OWN: the task that implements
# `mcf5307_exec` brings `mcf5307_exec` under this test with no edit here and
# no edit to `abi_smoke.cpp`. A hand-written list would need an edit per task
# and would silently stop growing the first time one was forgotten.
#
# A SET THAT SHRINKS CANNOT HIDE A RENAME. The worry about a measured set is
# that a renamed definition leaves the set and takes its own assertion with
# it. It cannot: step 4a stops the configure step over any name the shared
# object exports that the contract does not declare, so a definition renamed
# out of the contract fails the configure step before this file is read.
