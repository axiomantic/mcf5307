...
##
## THE MODEL IS NOT WIRED TO THE C ENTRY POINTS. `src/isp1181/stub.nim` carries
## those, and which implementation stands behind them is an OPEN OPERATOR
## DECISION: the design names no flag, no identifier, no type, no default and
## no owner for it. Choosing one here would answer a question that is
## deliberately left open, so this file supplies the model and selects nothing.
##
...
const fifoShape: array[fifoCount, tuple[capacity: int, buffers: int]] = [
  ...
  (16, 2),   ## endpoint 1
  (64, 2),   ## endpoint 2
  (64, 1)]   ## endpoint 3 - SINGLE. The authority marks double-buffering where
             ## it exists and leaves EP3 unmarked; the design document's
             ## "double" for this row is the error.
