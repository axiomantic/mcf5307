...
proc isp1181StateChecksum(buf: StateBuf; upTo: int): uint32 =
  ## FNV-1a over the first `upTo` bytes. Each step is a bijection on `uint32`,
  ## so two blocks of one length that differ anywhere reach different running
  ## values at the first difference and can never rejoin. Any single changed
  ## byte is detected, not with high probability but always.
...
proc isp1181Restore*(ctx: ISP1181Ctx; src: pointer): Isp1181StateStatus =
  ...
  ## reason it will not.
  ##
  ## Every check precedes the decode. The handle is written only after the block
  ## has been accepted whole, so a refusal leaves the caller with the state it
  ## had rather than with a handle half restored from a block that was never
  ## valid.
