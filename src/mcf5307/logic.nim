...
## `{decode_types, ea, machine}`. An executor that reaches into another
## executor for a helper rebuilds the decoder-under-executor cycle one layer
## down, and a shape that is merely awkward between a pair of siblings is
## unworkable across the whole group.
##
...
##       N and Z from the result, V and C cleared, X UNTOUCHED. V is defined as
##       an ARITHMETIC overflow, C as a carry out of an ADDITION or a borrow in
##       a SUBTRACTION, and X as taking C's value "for arithmetic operations;
##       otherwise not affected". A logical operation is none of those things.
##       That is exactly `setNzClearVc`, which `machine.nim` already holds for
##       the same rule under MOVE, and this module calls it rather than write a
##       second copy.
##
##   BTST, BSET, BCLR, BCHG
##       Z ALONE, and Z is the COMPLEMENT of the bit tested. The operation is
##       `~(<Bit Number> of Destination) -> Z` and names no other bit, so N, V,
##       C and X are left exactly as they were.
##
##   LSL, LSR, ASL, ASR
##       X AND C BOTH TAKE THE LAST BIT SHIFTED OUT, stated for every one of
##       them (`X/C <- (Dy << Dx) <- 0` and the two right-shift forms). N and Z
##       come from the result. V IS CLEARED BY EVERY ONE, ASL INCLUDED, and the
##       reference says so in its own words rather than by inference from the
##       word "arithmetic": V is given a flat "Always cleared" in the
##       condition-code table, with the note that CCR[V] is always cleared by
##       ASL and ASR, unlike on the 68K family processors, and the prose adds
...
##
##   X IS LEFT ALONE, AND THAT ONE IS REASONED. X takes the value of C "for
##   arithmetic operations; otherwise not affected", and a shift that moved no
##   bit produced no carry to copy. `execShift` guards X with `count != 0` for
##   exactly this.
##
...
##
##   3. WHETHER A DYNAMIC BTST MAY READ AN IMMEDIATE OPERAND. The timing table
##      dashes the `#xxx` column of the `btst Dy,<ea>` row, and
##      `m68k-elf-as -mcpu=5307` assembles `btst %d1,#5` anyway. The mask
##      follows the reference and traps it; the full evidence, including why
##      the assembler's acceptance is the 68000's rule rather than this part's,
##      is on `eaBitDynamic` in `decode_types.nim`.
##
##      TWO TABLES OF THE SAME REFERENCE DISAGREE, and that is stated here so a
##      reader can check it without leaving this list. The addressing-mode
...
##      instructions listed among the REMOVED ones.
##
##   5. THE REGISTER SHIFT COUNT'S MODULUS. `execShift` takes it modulo 64. The
##      shift operations are given as `X/C <- (Dy << Dx) <- 0` with no modulus
##      stated anywhere.
##
...
proc execShift(ctx: MCF5307Ctx; d: Decoded): uint32 =
  ...
  # The reference gives the shifts as `X/C <- (Dy << Dx) <- 0` and the two
  # right-hand forms and STATES NO MODULUS anywhere. It is uncertainty 5 in
