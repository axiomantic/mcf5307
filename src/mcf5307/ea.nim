...
## address whose mode is outside the mask is illegal and must trap. This is a
## mandatory property of the core: a permissive core hides a firmware fault by
## executing an addressing mode the silicon rejects.
##
...
## Manual and from this project's own measurements. No expression was taken
...
const
  ...
  # `eaDataAddressing`. The DATA category EXCLUDES `An`; this set does not, so a
  # reader must not take it for that class.
  #
  ...
  # EVERY VALID MODE-7 SUB-VARIANT, AND THE NAME SAYS SO. `ea7Unused5`,
  # `ea7Invalid` and `ea7Unused7` are encodings this part does not define, and
  # this set is the rest.
  #
  # THIS SET IS NOT THE DATA CLASS. `decode_types.nim` pairs it with
  ...
  #
  # WHY THE MODE LIST CANNOT RESTRICT ANYTHING HERE. Every mode but mode 7 is
  # alterable outright, and mode 7's sub-variants split between alterable and
  # not. A mode-level set therefore has to admit mode 7 to let the absolute
  # forms through, at which point it admits every mode and restricts nothing.
  # The split is a property of the register field alone.
  #
  ...
  # `mulu.l (4,%a0,%d2),%d1`. The masks those operations really carry are below.
