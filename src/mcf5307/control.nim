...
## import list is `{decode_types, ea, machine}`. An executor that reaches into
## another executor for a helper rebuilds the decoder-under-executor cycle one
## layer down, and a shape that is merely awkward between a pair of siblings is
## unworkable across the whole group. The one helper this group needed that no
## earlier group had - the exception stack frame - went DOWN into
## `machine.nim`, where `exception.nim` reaches it as a sibling.
##
...
##
##   Scc IS A BYTE. `Scc Dx` has an OPERAND SIZE of 8 and the operation "If
##   Condition True, Then 1's -> Destination; Else 0's -> Destination", so the
##   write REPLACES THE LOW BYTE of the data register and leaves the rest
##   alone. That is `mergeSized` in `machine.nim`, which `eaWrite` already
##   applies.
##
##   TST KEEPS ALL THREE SIZES, and it is the only instruction in this group
##   that does. It is given "8,16,32", and the timing table carries a `tst.b`
##   row, a `tst.w` row AND a `tst.l` row, each timed under every
##   effective-address column with no dash anywhere. `m68k-elf-as -mcpu=5307`
...
##
##   THE REFERENCE gives `Bcc <label>`, `BRA <label>` and `BSR <label>` an
##   OPERAND SIZE of "8,16" AND NO THIRD VALUE. A displacement byte of 0x00
##   selects the 16-bit form and a byte of 0xFF selects the 32-bit one; the
##   32-bit one is ISA_B and this part is ISA_A.
##
...
##       NO CONDITION CODE AT ALL. The OPERATION for each of them names the
##       program counter, the stack pointer or the destination and no flag.
##       `Bcc` and `Scc` READ the condition codes and write none.
##
##   TST
##       "Set Integer Condition Codes" at the operand size: N and Z from the
##       operand, V and C cleared, X UNTOUCHED. V is an ARITHMETIC overflow and
##       C is a carry out of an addition or a borrow in a subtraction, and a
##       test is neither. That is exactly `setNzClearVc`, which `machine.nim`
##       already holds for MOVE and which this module calls rather than write a
##       second copy.
##
##   CMP, CMPA, CMPI
##       A SUBTRACTION WITH THE RESULT DISCARDED. The operation is
...
##   way it pulls the flattened ones apart. `execScc` returns 1 and `scc Dx` is
##   1(0/0); `execRts` returns 8 and `rts` is 8(1/0); `execRte` returns 14 and
##   `rte` is 14(2/0); `execTrap` returns 18 and `trap #imm` is 18(1/2). An
##   exact number was not invented.
##
##   SOME ARE FLATTENED ACROSS THE EFFECTIVE ADDRESS. `execJump` returns 5 for
##   every operand; `jmp`/`jsr` is 5 for `(An)` and `(d16,An)` but 6 for the
##   indexed forms and 1 for `xxx.wl` - 1(0/0) for `jmp` and 1(0/1) for `jsr`,
##   whose extra write is the return address. The notes widen those `xxx.wl`
##   cells to 1-to-3, which the flat 5 is outside as well. `execTst` returns 1
##   for a register and 3 otherwise, and the flat 3 matches most of the
##   non-register cells of the `tst.l` row - `(An)`, `(An)+`, `-(An)`,
##   `(d16,An)` and `xxx.wl` are each 3(1/0) - but NOT the whole row:
##   `(d8,An,Xi*SF)` is 4(1/0) and `#xxx` is 1(0/0), and `tst.b` and `tst.w`
##   read 4(1/0) on those same cells. `execCompare` returns 1 for every
##   comparison; `cmpi.l #imm,Dx` has ONE cell and it is 1(0/0), SO THE FLAT 1
##   IS EXACT FOR CMPI, while the `cmp.l <ea>,Rx` row is 1(0/0) under `Rn` and
##   `#xxx` ALONE and reads 4(1/0) under `(An)`, `(An)+`, `-(An)`, `(d16,An)`
##   and `xxx.wl` and 5(1/0) under `(d8,An,Xi*SF)`.
##
##   ONE HAS NO SOURCE AT ALL, AND IT IS NOT A BRANCH. CMPA HAS NO TIMING ROW
##   ANYWHERE, so its number is invented outright rather than flattened out of a
##   row; `alu.nim` records the same absence for ADDA and SUBA. NO ROW ANYWHERE
##   is a worse fact than the branch returns below, which are outside a cell but
##   inside a documented range.
##
##   THE BRANCH RETURNS SIT INSIDE A DOCUMENTED RANGE, IN NO CELL.
##   `execBranch` returns 3 for BSR and 2 for BRA and Bcc. No cell carries
##   either: `bra` is 1(0/0) taken in either direction with both not-taken
##   columns dashed; `bsr` is 1(0/1) under `(d16,An)`/`(d16,PC)` with the rest
##   dashed; and Bcc reads 5(0/0) forward-taken, 1(0/0) forward-not-taken,
##   1(0/0) backward-taken and 5(0/0) backward-not-taken. THE NOTES BENEATH
##   THOSE TABLES ARE PART OF THEM. The notes on `bra`, on `jmp` and on `bsr`
##   each say the branch-acceleration decoupling makes the execution time "vary
##   from 1 to 3 cycles"; BSR's 3 and BRA's 2 are inside that range. Bcc's own
##   note runs past the end of its table onto the following page, which prints
##   the prediction algorithm and then a SECOND Bcc timing table: 1(0/0)
##   predicted correctly as taken, 1(0/0) predicted correctly as not-taken,
##   5(0/0) mispredicted. Immediately beneath it the
##   predicted-correctly-as-taken column "can vary between 1 to 3 cycles
##   depending on the amount of decoupling" between the two pipelines. Bcc's 2
##   is inside that range, by the same reading that puts BRA's 2 inside its own
##   note's.
##
##   A TABLE'S ROWS AND ITS NOTES CAN END ON DIFFERENT PAGES, and a second
##   table on the same subject can follow the notes. A reading of a table taken
##   from one page is not complete until the next page has been read.
##
...
##      the condition-code section ends with the unattached sentence "Set to
##      the value of the C-bit for arithmetic operations; otherwise not
...
##      compared between its steps. It is stated as a property of the X BIT and
##      not as a per-instruction rule, CMP is given no flag column at all, and
##      this module follows the family rule.
##
...
## measurements with the pinned cross assembler. No expression was taken from
## any copyleft source.
...
proc execBranch(ctx: MCF5307Ctx; word: uint16; d: Decoded;
  ...
  if d.op == opBsr:
    # "SP - 4 -> SP; PC -> (SP); PC + dn -> PC". The pushed value is the
    # address after the WHOLE instruction, so the byte form pushes two bytes on
    # and the word form four.
    ctx.sp = ctx.sp - 4'u32
...
proc execTst(ctx: MCF5307Ctx; d: Decoded): uint32 =
  ...
  ##
  ## THE REFERENCE DOES NOT SEPARATE THEM. The `tst` rows carry a time under a
  ## column headed `Rn`, and `Rn` is defined as "Any Address or Data Register"
  ## - so that column cannot say which of the two a given size admits. The
  ## `clr.b` row above uses the
...
proc execJump(ctx: MCF5307Ctx; d: Decoded; insnPc: uint32): uint32 =
  ...
  ## what "SP - 4 -> SP; PC -> (SP)" means by PC. A
...
proc execRte(ctx: MCF5307Ctx; insnPc: uint32): uint32 =
  ...
  ## 4-bit format field to validate the frame type", and "any attempted
  ## execution of an RTE where the format is not equal to {4,5,6,7} generates a
  ## format error". This core TRAPS instead of taking vector 14; that is
  ## uncertainty 4 in this module's header.
  ##
  ## THE STACK POINTER IS `SP + 4 + FORMAT` AND NOT `SP + 8`. The processor
  ## "adjusts the stack pointer by adding the format value to the
  ## auto-incremented address after the fetch of the first longword", which is
  ## SP + 4 plus 4, 5, 6 or 7. That is the inverse of the frame layout, which
  ## puts the handler's A7 at the original A7 minus 8, 9, 10 or 11. A core that
  ## added a fixed 8 restores the wrong pointer for most of the frames a
  ## misaligned stack produces.
  ##
  ## THE INSTRUCTION SUMMARY'S `RTE` ROW READS "SP + 8 -> PC" AND THAT IS A
  ## MISPRINT. The program counter has just been loaded from (SP+4) and the row
  ## would overwrite it with an address on the stack. The prose passage states
  ## the rule properly and it is what this follows.
