...
# never a prebuilt `.a`. The integration steps run here, in order, and each one
...
# The prefix is read off the command that RUNS rather than declared a second
# time, so the scaffolding names below cannot drift from the flag the Nim
# compiler was actually given.
mcf5407_render_command(MCF5407_NIM_BUILT_COMMAND_TEXT ${MCF5407_NIM_COMMAND})
string(REGEX MATCH "--nimMainPrefix:([^ ]+)"
    MCF5407_NIM_BUILT_PREFIX_MATCH "${MCF5407_NIM_BUILT_COMMAND_TEXT}")
set(MCF5407_NIM_BUILT_PREFIX "${CMAKE_MATCH_1}")
...
foreach(index RANGE ${MCF5407_NIM_LAST_UNIT})
    ...
    file(TO_CMAKE_PATH "${MCF5407_NIM_UNIT}" MCF5407_NIM_UNIT)
...
if(MCF5407_NIM_DUMP_RESULT EQUAL 0)
    ...
    if(MCF5407_NIM_DUMP_JSON_ERROR STREQUAL "NOTFOUND"
        ...
        file(TO_CMAKE_PATH "${MCF5407_NIM_DUMP_LIBPATH}" MCF5407_NIM_LIB_DIR)
        set(MCF5407_NIM_LIB_DIR_SOURCE
...
#
# `-DMCF5407_ABI_GATE=OFF` configures such a host. It skips the visibility
# measurement and the four controls that prove the measurement can still fail,
# and the warning it prints says so.
#
# The warning is not an enforcement. `message(WARNING)` fails neither `cmake`,
# nor `cmake --build`, nor `ctest`, and the switch is a `CACHE BOOL`, so a build
# directory configured OFF once reads OFF back on every later configure with
# nobody naming the switch again. A tree configured OFF measures nothing here
# and reports that only in scrollback.
set(MCF5407_ABI_GATE ON CACHE BOOL
    "Run step 4a. cmake/Nim.cmake states what OFF skips")

if(NOT MCF5407_ABI_GATE)
    message(WARNING
        "mcf5407: step 4a IS TURNED OFF. MCF5407_ABI_GATE is OFF.\n"
        "  THE VISIBILITY GATE. Nothing in this configure run measured whether "
        "the symbols ${MCF5407_ABI_CONTRACT_FILE} publishes leave the shared "
        "object with default visibility. A published symbol that reaches the "
        "shared object hidden makes a plugin that exports nothing, and this "
        "build would not report it.\n"
        "  ITS FOUR CONTROLS, A THROUGH D, ARE SKIPPED TOO. Those are what "
        "prove on every configure run that the measurement can still fail, so "
        "with the gate off nothing reports a gate that has stopped working "
        "either.\n"
        "  `abi_smoke` FALLS BACK TO ONE NAME. The address set of "
        "tests/abi_smoke.cpp is generated from this step's measurement, so "
        "with the gate off that test takes the address of "
        "`mcf5407_runtime_init` alone.\n"
        "THIS MESSAGE IS NOT AN ENFORCEMENT. It fails neither cmake, nor the "
        ...
        "being named again. Turn the gate back on with "
        "-DMCF5407_ABI_GATE=ON.")
else()
 ...
 *
 * The first group holds the shapes that defeated the regular-expression
