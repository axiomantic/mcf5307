...
byte-identical regeneration check depends on the assembler being the pinned
version. A different assembler version can emit different bytes for the same
source, so the pinned version is recorded here, beside `.nim-version`, and the
Linux x86-64 CI job installs exactly it.
...
In brief: one file per group (`move`, `alu`, `logic`, `control`), each with a
non-empty `cases` array, and every
